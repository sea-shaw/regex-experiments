...
import experiments.macros.oregano.Regex
import org.scalatest.flatspec.AnyFlatSpec
import org.scalatest.matchers.should.Matchers.{matchPattern, should}

class OreganoUnitTests extends AnyFlatSpec {

  behavior of "Oregano regex macro"
  ...
  it should "match the left capture group in one or more alternatives" in {
    val r = Regex("((a)|(b))+")
    "a" should matchPattern { case r("a", Left(Left("a"))) => }
  }

  it should "match the right capture group in one or more alternatives" in {
    val r = Regex("((a)|(b))+")
    "b" should matchPattern { case r("b", Left(Right("b"))) => }
  }

  it should "match both capture groups in one or more alternatives" in {
    val r = Regex("((a)|(b))+")
    "ba" should matchPattern { case r("a", Right("a", "b")) => }
    "ab" should matchPattern { case r("b", Right("a", "b")) => }
  }

  it should "match the left capture group in zero or more alternatives" in {
    val r = Regex("((a)|(b))*")
    "a" should matchPattern { case r(Some("a", Left(Left("a")))) => }
    "" should matchPattern { case r(None) => }
  }

  it should "match the right capture group in zero or more alternatives" in {
    val r = Regex("((a)|(b))*")
    "b" should matchPattern { case r(Some("b", Left(Right("b")))) => }
    "" should matchPattern { case r(None) => }
  }

  it should "match both capture groups in zero or more alternatives" in {
    val r = Regex("((a)|(b))*")
    "ba" should matchPattern { case r(Some("a", Right("a", "b"))) => }
    "ab" should matchPattern { case r(Some("b", Right("a", "b"))) => }
    "" should matchPattern { case r(None) => }
  }
}
